...
@MainActor
public protocol SAFormCollectionItem {}

@MainActor
@Observable
public final class SAFormCollection<Item: SAFormCollectionItem> {
    ...
    private func makeItem() -> Item {
        guard let itemFactory else {
            preconditionFailure(
                "SAFormCollection: itemFactory is not configured. Use init(_ itemFactory:) or add(_ item:)."
            )
        }
    ...
    private func ensureIndexExists(_ index: Int) {
        precondition(index >= 0, "SAFormCollection: index must be greater than or equal to 0.")
