...
public extension SAFormValidationRules {
    ...
    func floating() -> SAFormFloatingValidation<Float> {
        .init()
    }
    ...
    /// - Returns: A floating-point validation builder for chaining rules.
    func floating<T: BinaryFloatingPoint & Sendable>(_ type: T.Type) -> SAFormFloatingValidation<T> {
        .init()
    }
}

/// A validation builder for floating-point values that supports composing multiple rules.
public struct SAFormFloatingValidation<T: BinaryFloatingPoint & Sendable>: SAFormValidationTypeRules {
